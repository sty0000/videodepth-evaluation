...
def load_images_as_tensor(path='data/truck', num_images=None, interval=1, PIXEL_LIMIT=255000, save_rgb_path=None):
    """
    Loads images from a directory or video, resizes them to a uniform size,
    then converts and stacks them into a single [N, 3, H, W] PyTorch tensor.
    If save_rgb_path is provided, saves the resized images.
    """
    sources = [] 
    ...
    if osp.isdir(path):
        ...
        for i in range(0, len(filenames), interval):
            if num_images and len(sources) >= num_images:
                break
    ...
    if save_rgb_path is not None:
        rgb_path = osp.join(save_rgb_path, "rgb")
        os.makedirs(rgb_path, exist_ok=True)

    # --- 4. Resize images, convert to tensors, and save resized images ---
    tensor_list = []
    to_tensor_transform = transforms.ToTensor()
    
    for idx, img_pil in enumerate(sources):
        try:
            # Resize to the uniform target size
            resized_img = img_pil.resize((TARGET_W, TARGET_H), Image.Resampling.LANCZOS)
            
            # Save the resized image if path is provided
            if save_rgb_path is not None:
                output_path = osp.join(rgb_path, f"frame_{idx:06d}.jpg")
    ...
    # --- 5. Stack the list of tensors into a single [N, C, H, W] batch tensor ---
