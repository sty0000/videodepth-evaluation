    ...
    for idx_dataset, dataset_name in enumerate(all_eval_datasets, start=1):
        ...
        if dataset_info.type == "video":
            ...
            def get_sampled_list(full_list):
                if max_frames is not None and len(full_list) > max_frames:
                    indices = np.linspace(0, len(full_list) - 1, max_frames, dtype=int)
                    return [full_list[i] for i in indices]
                return full_list

            raw_gt_dict = {seq: list_depths_a_sequence(dataset_info, seq) for seq in seq_list}
            raw_pd_dict = {seq: sorted(glob.glob(osp.join(output_root, seq, "*.npy"))) for seq in seq_list}

            gt_paths = {}
            pred_paths = {}
            for seq in seq_list:
                sampled_gt = get_sampled_list(raw_gt_dict[seq])
                if len(sampled_gt) == len(raw_pd_dict[seq]) and len(sampled_gt) > 0:
                    gt_paths[seq] = sampled_gt
                    pred_paths[seq] = raw_pd_dict[seq]
                else:
                    logger.warning(f"Skipping {seq}: GT({len(sampled_gt)}) != Pred({len(raw_pd_dict[seq])})")

            if len(gt_paths) == 0:
                raise ValueError(f"No valid sequences found for dataset {dataset_name}. Check paths or max_frames.")
        elif dataset_info.type == "squid":
            from utils.files import get_squid_image_sets, list_squid_depth_an_image_set
            ...
            for scene in dataset_info.ls_all_seqs:
                ...
                for image_set in image_sets:
                    seq_key = f"{scene}/{image_set}"
                    gt_path = list_squid_depth_an_image_set(dataset_info, scene, image_set)
                    pred_path = osp.join(output_root, scene, image_set, "frame_0000.npy")  # 左图深度
                    if gt_path and osp.exists(pred_path):
                        gt_paths[seq_key] = [gt_path]  # 只有一个深度GT
                        pred_paths[seq_key] = [pred_path]      
        else:
