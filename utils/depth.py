...
EVAL_DEPTH_METADATA = {
    ...
    "flsea-canyons": {
        "depth_read_func": lambda filename: depth_read_flsea(filename),
        "depth_evaluation_kwargs": {
            "max_depth": None,
            "use_gpu": True
        },
    ...
    "usod10k": {
        "depth_read_func": lambda filename: depth_read_usod10k(filename),
        "depth_evaluation_kwargs": {
            "max_depth": None,
            "use_gpu": True
        },
    ...
    "seathru": {
        "depth_read_func": lambda filename: depth_read_seathru(filename),
        "depth_evaluation_kwargs": {
            "max_depth": None,
            "use_gpu": True
        },
    ...
    "squid": {
        "depth_read_func": lambda filename: depth_read_squid(filename),
        "depth_evaluation_kwargs": {
            "max_depth": None,
            "use_gpu": True
        },
...
def depth_read_flsea(filename: str):
    depth = tiff_imread(filename).astype(np.float64)
    depth[depth == 0] = -1.0
    return depth

def depth_read_usod10k(filename: str):
    img_pil = Image.open(filename)
    depth_png = np.array(img_pil)    
    max_val = depth_png.max()
    depth = (max_val - depth_png).astype(float) / 65535
    # depth = depth_png.astype(np.float64)
    depth[depth_png == 0] = -1.0
    return depth

def depth_read_seathru(filename: str):
    depth = tiff_imread(filename).astype(np.float64)
    depth[depth < 0.1] = -1.0
    return depth

def depth_read_squid(filename: str):
    """Read SQUID depth from .mat file (distanceFromCamera.mat)"""
    dist_data = scipy.io.loadmat(filename)
    var_name = [k for k in dist_data.keys() if not k.startswith('__')][0]
    depth = dist_data[var_name].astype(np.float64)
...
def depth_evaluation(
    ...
    mae= torch.mean(torch.abs(predicted_depth - ground_truth_depth)).item()
